...
/// <summary>
///     Lookup helper for known language ISO codes (e.g. "en-US") and their display names.
/// </summary>
public static class LanguageIsoCodeHelper
{
    ...
    /// <summary>
    ///     Return true if the ISO code is a known, supported language code.
    /// </summary>
    ...
    /// <summary>
    ///     Return the display name for the given ISO code, or null if not found.
    /// </summary>
    public static string? GetName(string isoCode)
    {
        return AvailableLanguages.TryGetValue(isoCode, out var name) ? name : null;
    }

    /// <summary>
    ///     Return the ISO code matching the given display name, or null if not found.
    /// </summary>
    public static string? GetCode(string codeName)
    {
        foreach (var pair in AvailableLanguages)
        {
            if (string.Equals(pair.Value, codeName, StringComparison.Ordinal))
            {
                return pair.Key;
            }
        }
