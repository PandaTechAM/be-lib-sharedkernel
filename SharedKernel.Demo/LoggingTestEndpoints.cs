...
public class LoggingTestEndpoints : IEndpoint
{
    public void AddRoutes(IEndpointRouteBuilder app)
    {
        ...
        grp.MapGet("/no-content-type",
                async ctx => { await ctx.Response.Body.WriteAsync("raw body with no content-type"u8.ToArray()); })
        ...
        grp.MapGet("/outbound/binary", async (IHttpClientFactory factory) =>
            {
                ...
                return Results.Ok(new
                    { statusCode = (int)response.StatusCode, length = response.Content.Headers.ContentLength });
            })
